...
function rollForTurn() {
    ...
    if (pOne > pTwo) {
        first = "Player 1";
        setTimeout(function() { txt1 = txt1 + "Player 1 wins, please choose a square.";}, 2000); // needs to be one s later than first written element
        setTimeout(function() {writeMsg(txt1);}, 2000);
    }   else if (pOne < pTwo) {
        first = "Player 2";
        setTimeout(function() { txt1 = txt1 + "Player 2 wins, please choose a square.";}, 2000); // needs to be one s later than first written element
        setTimeout(function() {writeMsg(txt1);}, 2000);
    }
...
function stopEnabled(btn) {
    btn.style.color = "fff";
    btn.style.border = "2px solid rgb (201, 0 , 0)";
...
function recordMoves(square) {
    ...
    verdict = check(info,square); // call function to check if propsed square is already occupied
    return verdict;
}
...
// call this function to check boardstate for any ties and act accordingly
function check4Tie() {
    var boardState = document.getElementById('boardState').innerHTML;
    boardState = boardState.substring(1); // remove leading comma
    boardState = boardState.split(','); // seperat the string by commas into an array
    var check= document.getElementById('gameMsg').innterHTML;
...
// call this function to produce the following wining process for the game
...
function glowBoard(pos) {
    ...
    for (var i=0; 1<square.length; i++) {
        if (i == index0) {
            ...
            var bg2 = squares[i];
            setTimeout(function() {bg2.style.backgroundColor = 'rgb(66, 234, 235)';}, 100);
...
// call this function to make the entire background color
...
function blink() {
    ...
    setTimeout(function() {body.style.backgroundColor = '#94e6f7';}, 300);
...
//------------------------------------------------------------------------
// These function are the algorithms to find all the win conditions
//-------------------------------------------------------------------------
...
function checkWinCon1(info,squareArray) {
    ...
    // avatars and compare all 2 for win cons
    for (var i in info) {
...
function checkWinCon2(info,squareArray) {
    ...
    if (match3Avatar != undefined && match4Avatar != undefined && match5Avatar != undefined) { // this will trigger (ONLY) if there was a match for index 3, index 4, and index .5
...
function checkWinCon4(info,squareArray) {
    ...
    for (var i in info) {
        ...
        if (info[i].charAt(0) == "6") {
            var match6Avatar = info[i].charAt(1);// NOT SURE WHY THIS WAS MISSING FROM THE TUTORIAL!!!!!!!!!!!
        }
    }
